...
List available covariates. We have available the monthly aggregation of Degree Heating Weeks (DHW).
...
Focus on degree heating weeks (DHW), and get the max (of the monthly average) for the 365 days prior to the survey data. Review the survey data:
...
The `get_zonal_statistics()` function takes the site latitude and longitude, as well as the survey date, to find the data at that site for `n` days prior, then aggregates it. The buffer size is set to 1000 metres.
