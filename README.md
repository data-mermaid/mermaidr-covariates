...
To find which covariates are available, we use the `list_covariates()`
function:
...
function `get_zonal_statistics()`. The `get_zonal_statistics()` function
takes the site latitude and longitude, as well as the survey date, to
find the data at that site for `n` days prior, then aggregates it.
